...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      float flareSize =
        smoothstep(0.9, 1.0, size) * glossLocal;
...
void main() {
  ...
  if (uAutoCenterRepulsion > 0.0) {
    ...
    vec2 repulsion =
      normalize(direction + vec2(0.0001)) *
      (
        uRepulsionStrength /
        (mouseDist + 0.1)
      );
...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    /*
     * IMPORTANT:
     * Mouse tracking is attached to WINDOW,
     * not the Galaxy container.
     *
     * This allows the Galaxy to react even
     * when the mouse is over the login panel.
     */
